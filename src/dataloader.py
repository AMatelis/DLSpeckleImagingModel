...
def load_video_frames(video_path: str, target_resolution: Tuple[int, int]) -> List[np.ndarray]:
    """Load grayscale frames from video file and resize."""
...
class SpeckleDataset(Dataset):
    ...
    def _apply_augmentation(self, clip: torch.Tensor) -> torch.Tensor:
        ...
        if np.random.rand() < 0.5:  # brightness
            clip *= 1.0 + np.random.uniform(-0.1, 0.1)
...
def prepare_dataset(
    ...
    """Create dataset of video stacks and labels with consistent shape."""
    ...
    for filename in sorted(os.listdir(data_folder)):
        ...
        for i in range(0, len(frames) - sequence_len + 1, stride):
            clip = np.stack(frames[i:i + sequence_len], axis=0).astype(np.float32)
